...
docker build --no-cache --build-arg COMMIT=$commit --build-arg BUILD_TIME=$bt -t bitemp-go-api:v05.01.03 .
```
...
```powershell
docker run -d --name bitemp-go-api-v05 \
  -p 8081:8080 \
  -e DATABASE_URL="postgres://postgres:1234@host.docker.internal:5432/bitemp_go_db_v05?sslmode=disable" \
  -e APP_ENV=production \
  -e GIN_MODE=release \
  -e ALLOW_DROP_TABLES=false \
  -e ADMIN_DROP_PASSWORD="kies-een-sterk-geheim" \
  --restart unless-stopped \
  bitemp-go-api:v05.01.03
```

In 1 regeL:
```
docker run -d --name bitemp-go-api-v05 -p 8081:8080 -e DATABASE_URL="postgres://postgres:1234@host.docker.internal:5432/bitemp_go_db_v05?sslmode=disable" -e APP_ENV=production -e GIN_MODE=release -e ALLOW_DROP_TABLES=false -e ADMIN_DROP_PASSWORD="1234" --restart unless-stopped bitemp-go-api:v05.01.03
```
...
# Logs
docker logs --tail 200 bitemp-go-api-v05
...
```powershell
# 1) Nieuwe image bouwen (voorbeeld tag v05.01.03)
$commit = (git rev-parse --short HEAD)
$bt = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
docker build --no-cache --build-arg COMMIT=$commit --build-arg BUILD_TIME=$bt -t bitemp-go-api:v05.01.03 .

# 2) Oude API stoppen/verwijderen
docker rm -f bitemp-go-api-v05
...
```powershell
docker save -o bitemp-go-api_v05.01.03.tar bitemp-go-api:v05.01.03
```

Kopieer `bitemp-go-api_v05.01.03.tar` naar de doelmachine.

NAS: \\TRUENAS\share\Docker\Bitemporal\bitemp-go-api_v05.01.03.tar


### Op doelmachine

```powershell
docker load -i bitemp-go-api_v05.01.03.tar
docker run -d --name bitemp-go-api-v05 -p 8081:8080 -e DATABASE_URL="postgres://USER:PASSWORD@DB_HOST:5432/DB_NAME?sslmode=require" -e APP_ENV=production -e GIN_MODE=release -e ALLOW_DROP_TABLES=false -e ADMIN_DROP_PASSWORD="sterk-geheim" --restart unless-stopped bitemp-go-api:v05.01.03
```
...
```powershell
docker tag bitemp-go-api:v05.01.03 <registry-user>/bitemp-go-api:v05.01.03
docker push <registry-user>/bitemp-go-api:v05.01.03
```
...
```
docker tag bitemp-go-api:v05.01.03 markwestbroek/bitemp-go-api:v05.01.03
...
```powershell
docker pull <registry-user>/bitemp-go-api:v05.01.03
docker rm -f bitemp-go-api-v05
docker run -d --name bitemp-go-api-v05 -p 8081:8080 -e DATABASE_URL="postgres://..." -e APP_ENV=production -e GIN_MODE=release -e ALLOW_DROP_TABLES=false -e ADMIN_DROP_PASSWORD="kies-een-sterk-geheim" --restart unless-stopped <registry-user>/bitemp-go-api:v05.01.03
```
...
```bash
docker logs --tail 100 bitemp-postgres-v05
docker exec -it bitemp-postgres-v05 pg_isready -U postgres -d bitemp_go_db_v05
```
...
Belangrijk:
- In deze opzet gebruikt de API als DB hostnaam: `bitemp-postgres-v05`.
...
API upgraden:
1. Push nieuwe image tag (bijv. `v05.01.03`).
2. Pas `image: markwestbroek/bitemp-go-api:v05.01.03` aan in de API stack.
...
Melding:
`Conflict. The container name "/bitemp-go-api-v05" is already in use`
...
```bash
docker rmi markwestbroek/bitemp-go-api:v05.01.03
```
