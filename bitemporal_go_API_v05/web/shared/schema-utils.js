(function initSchemaShared(windowObj) {
  ...
  function donkerdereRandkleurVanHex(hexKleur, ratio) {
    const effectieveRatio = typeof ratio === "number" ? ratio : 0.62;
    ...
    const donker = (n) => Math.max(0, Math.min(255, Math.round(n * effectieveRatio)));
  ...
  function leidEntiteitTypeAfUitKolomnaam(kolomnaam, beschikbareEntiteitTypen) {
    ...
    const bestaat = safeArray(beschikbareEntiteitTypen)
      .some((item) => String(item?.typenaam || "").toUpperCase() === prefix);
    return bestaat ? prefix : "";
  }

  windowObj.SchemaShared = {
    safeArray,
