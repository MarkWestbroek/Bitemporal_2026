...
| **Auto-layout (alles)** | Toolbar: `🎯 Auto-layout` of rechtsklik op canvas → `🎯 Auto-layout (alles)`. Ordent alle (zichtbare) nodes hiërarchisch: ENT centraal/bovenaan, GE's eronder (rij of grid; ring bij ≥ 8 GE's), enums/datatypes/reflijsten als kleine boxen **direct onder hun consumer-GE in dezelfde kolom** (REL-eigen secs onder de REL zelf), RELs op halverwege tussen hun ENT-uiteinden, ankers tussen REL en ENT. ENT-clusters worden per **domein** gegroepeerd in eigen blokken; binnen een domein worden ENT's gesorteerd op netto-flow (bron→doel) zodat REL-pijlen zoveel mogelijk van links naar rechts lopen. **Hubs** met veel cross-domein verbindingen krijgen automatisch een eigen ringmodus (RELs op een cirkel rond de hub-ENT). Na de hiërarchische pass volgt een lichte **force-directed nabewerking** (springs + Coulomb-repulsie, ~80 iteraties) die overlappingen verder spreidt. Daarna worden alle relaties automatisch genormaliseerd. Een undo-snapshot wordt bewaard, dus `Ctrl + Z` zet de oude posities terug. Algoritme in [`metamodel/autoLayout.js`](metamodel/autoLayout.js). |
| **Auto-layout (selectie)** | Selecteer 2+ nodes → rechtsklik → `🎯 Auto-layout (selectie)`. Werkt identiek aan auto-layout, maar reorganiseert alléén de geselecteerde sub-graaf. Het resultaat wordt **scale-to-fit** ingevuld in de oorspronkelijke selectie-bounding-box (vult `vulSelectie`-ruimte op tot factor 2.5×) en daar gecentreerd zodat de rest van het diagram visueel op zijn plek blijft. Edges worden alleen genormaliseerd als ze ten minste één eindpunt in de selectie hebben. |
| **Layout-richting** | Toolbar dropdown naast `🎯 Auto-layout`: kies tussen `↓ Top→Bottom` (default), `↑ Bottom→Top`, `→ Left→Right`, `← Right→Left` en `⊙ Radial`. Wordt onthouden in `localStorage`. De keuze geldt voor zowel `Auto-layout (alles)` als `Auto-layout (selectie)` en wordt na de hiërarchische pass toegepast als geometrische transformatie (radial: hoek = horizontale positie, radius = verticale positie). |
| **Sticky / locked nodes** | Een node met `data.layoutLocked === true` wordt door auto-layout (alles én selectie) overgeslagen en behoudt zijn positie. Andere nodes laten zich daar gewoon omheen plaatsen. Toggle via rechtsklik op selectie → `🔒 Vergrendel positie` / `🔓 Ontgrendel positie`. De lock-status is **persistent over de volledige roundtrip**: in de UML editor reist `layoutLocked` mee in de V3 JSON op elk node-type (entiteit, GE, relatie + anker via `ankerLayoutLocked`, enum, gegevenstype, referentielijst-instantie, notitie, constraint); de backend bewaart het in `EditorLayout.LayoutLocked`/`AnkerLayoutLocked` (`model/metaregistry_plumbing.go`); codegen emit het in de gegenereerde `_metaregistry.go`/`_enum_registry.go`/`_datatype_registry.go`; en `v3_exporter.go` schrijft het terug naar V3 zodat editor ↔ V3 ↔ codegen ↔ V3 ↔ editor de lock behoudt. In de IDE wordt `layoutLocked` per diagram op `DiagramNode` opgeslagen, zodat 🔒 een tab-wissel of reload overleeft én per diagram kan verschillen. |
| **Notities/constraints bij hun onderwerp** | Auto-layout plaatst notities en constraints met een dependency-edge naar één of meer subjects rechts naast het zwaartepunt van die buren (in plaats van rechts van het hele diagram). Notities zonder edges vallen terug op de oude rechts-stapel. |
