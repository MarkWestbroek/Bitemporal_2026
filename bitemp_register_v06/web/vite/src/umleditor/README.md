...
| **Auto-layout (alles)** | Toolbar: `🎯 Auto-layout` of rechtsklik op canvas → `🎯 Auto-layout (alles)`. Ordent alle (zichtbare) nodes hiërarchisch: ENT centraal/bovenaan, GE's eronder (rij of grid; ring bij ≥ 8 GE's), enums/datatypes/reflijsten als kleine boxen onder de bijbehorende GE, RELs op halverwege tussen hun ENT-uiteinden, ankers tussen REL en ENT. ENT-clusters worden per **domein** gegroepeerd in eigen blokken; binnen een domein worden ENT's gesorteerd op netto-flow (bron→doel) zodat REL-pijlen zoveel mogelijk van links naar rechts lopen. **Hubs** met veel cross-domein verbindingen krijgen automatisch een eigen ringmodus (RELs op een cirkel rond de hub-ENT). Na de hiërarchische pass volgt een lichte **force-directed nabewerking** (springs + Coulomb-repulsie, ~80 iteraties) die overlappingen verder spreidt. Daarna worden alle relaties automatisch genormaliseerd. Een undo-snapshot wordt bewaard, dus `Ctrl + Z` zet de oude posities terug. Algoritme in [`metamodel/autoLayout.js`](metamodel/autoLayout.js). |
