...
| **Auto-layout (alles)** | Toolbar: `🎯 Auto-layout` of rechtsklik op canvas → `🎯 Auto-layout (alles)`. Ordent alle (zichtbare) nodes hiërarchisch: ENT centraal/bovenaan, GE's eronder (rij of grid; ring bij ≥ 8 GE's), enums/datatypes/reflijsten als kleine boxen onder de bijbehorende GE, RELs op halverwege tussen hun ENT-uiteinden, ankers tussen REL en ENT. ENT-clusters worden per **domein** gegroepeerd in eigen blokken. Daarna worden alle relaties automatisch genormaliseerd. Een undo-snapshot wordt bewaard, dus `Ctrl + Z` zet de oude posities terug. Algoritme in [`metamodel/autoLayout.js`](metamodel/autoLayout.js). |
