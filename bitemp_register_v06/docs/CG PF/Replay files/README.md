...
| 16 | `registraties-replay-init-formulierdefinitie-aanmelding-initiatief-2026-09-24.json` | platform | frontend van na 24-09-2026 (nieuw-modus): FormulierDefinitie **2** "Aanmelding initiatief" (30 vragen van het MS Forms-aanmeldformulier als layout met `vasteWaarde`, `kopieerNaar`, vaste rijen en meerkeuze). Onafhankelijk van 1–15; kies het formulier op `inhoud.html#/t/initiatieven/nieuw?formulier=2`. Vereist dat FormulierDefinitie-id 2 vrij is (anders het id in het bestand aanpassen) |
| 17 | `registraties-replay-init-formulierdefinitie-nieuwe-organisatie-2026-09-25.json` | platform | 16 en een backend met plaatshouder-id's (stap B, 25-09-2026): FormulierDefinitie **3** "Nieuwe organisatie" (ingebed subformulier) en layout v2 van FD 2 met `nieuwFormulier: "3"` op de organisatie-velden van vraag 10 en 13 |
| 18 | `registraties-replay-correctie-organisaties-opschoning-2026-09-25.json` | platform | 4–7, 13 en een backend met plaatshouder-id's: opschoning van de organisaties uit de intake — 11 tekstflarden ('pas toe of leg uit', hosting, cateraars, …) afgevoerd met hun relatie (A), 16 samengestelde/dubbele namen omgehangen naar canonieke organisaties met 5 nieuwe (Acato, Nerds&Company, Vebego Groen, Cadac, Analyze B.V.; via `$nieuw`) (B), Haarlem/Súdwest-Fryslân → "Gemeente …", PO-contactpersoon van Utrecht (hoort bij Contezza) afgevoerd. Afvoer op `initiatief_id` + `rel_id`; die zijn gelijk op instanties met dezelfde replay-volgorde — vooraf gecontroleerd op pf. Entries zijn `registratie` (in een `correctie` kan geen nieuwe entiteit worden opgevoerd), alleen de twee naamcorrecties zijn `correctie` |
| 19 | `registraties-replay-correctie-formulierdefinitie-aanmelding-meerkeuze-2026-09-25.json` | platform | 17: layout v3 van FD 2 — domeinen, gemeenten (beide rollen) en API-standaarden als `widget: meerkeuze` (chips + zoekveld, `RefMeerkeuze`); voert layout-hub rel_id 2 af |
| 20 | `registraties-replay-correctie-formulierdefinitie-aanmelding-tekst-2026-09-25.json` | platform | 16: beschrijving van FD 2 voor bezoekers (staat boven het openbare formulier); voert meta-hub rel_id 1 af |
| 21 | `registraties-replay-init-querydefinitie-nieuwe-aanmeldingen-2026-09-25.json` | platform | 10, 11: QueryDefinitie 3 `nieuwe-aanmeldingen` (**intern**): de modereerlijst — initiatieven met status `nieuwe_aanmelding` of `in_behandeling`. Uitvoeren: `POST /graphql/query {"documentId":"nieuwe-aanmeldingen"}` als ingelogde gebruiker (minimaal viewer). Basis voor de latere DashboardDefinitie-tegel en de notificatie-inhoud. Let op: de materiële aanvang (`querydefinitie_aanvang.datum`) telt in UTC — een replay met de datum van vandaag is op een UTC-server pas na 02:00 CEST geldig; neem in replays de datum van gisteren |
| 22 | `registraties-replay-init-notificatiedefinitie-nieuwe-aanmelding-2026-09-25.json` | platform | backend met `NotificatieDefinitie` (configuratie-model 25-09-2026) en `SMTP_*`/`NOTIFICATIE_*` in `.env`: NotificatieDefinitie 1 `nieuwe-aanmelding-melden` — e-mail aan de moderatoren bij een registratie op Initiatief met bron `aanmeldformulier`. **Adres is een placeholder** (`moderatie@common-ground-lab.nl`): corrigeer het abonnee-record. Bezorglog: `GET /notificaties/bezorgingen` (admin) |
| 23 | `registraties-replay-init-dashboarddefinitie-moderatie-2026-09-25.json` | platform | 11, 21 en een backend met `DashboardDefinitie` (configuratie-model 25-09-2026): dashboard `moderatie` — tellers (openstaand, publiek) en de tabel *Te beoordelen*. Openen: `/viz/react/dashboard.html?dashboard=moderatie`, ingelogd (minimaal viewer) |
