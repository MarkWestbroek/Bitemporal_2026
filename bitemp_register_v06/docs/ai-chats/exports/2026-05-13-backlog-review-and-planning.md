...
Ik ga twee dingen doen parallel: (1) nieuwe brainstorm-items aan  toevoegen, (2) een nieuw plan-document `BACKLOG_UITVOERING_INCREMENTEN.md` maken met de uitgebreide plannen. 
```
...
```
Nu het plan-document maken:  Klaar. Twee dingen gedaan:

### 1. Plan-document gemaakt
 — gestructureerd rond jouw accenten:
...
Wil je nu meteen die handmatige pass doen (ik kan systematisch  aflopen en per item checken in `RELEASE.md` + recente git-log + code), of liever eerst aan een ander accent beginnen?
...
Goede vragen — laat me eerst even snel kijken hoe de frontend nu valideert en of MetaRegistry al iets van veld-info heeft (twee dingen die mijn antwoord substantieel beïnvloeden), dan formuleer ik een concreet voorstel.   Goed beeld nu. Hier is mijn analyse en voorstel — ik wacht op groen licht voordat ik refactor.

## 1. Bevindingen uit de codebase

**Frontend doet validatie al grotendeels generiek** in web/vite/src/umleditor/validatie/regels.js + :
- `checksum` →  evalueert de regel zoals "(9*d1 + 8*d2 + … - d9) % 11 == 0" rechtstreeks. **Geen BSN-hardcoding**.
- `formula` → idem met  als  als parameter.
- `function` → kleine dispatch-table  — dít is wel hardcoded, maar bewust als "named library" voor dingen die te lastig zijn om als expressie te schrijven.
...
- Schrijf in Go een **expressie-evaluator** die `checksum`-regels (`d1..dn` placeholders + arithmetiek + `%`, `==`) en `formula`-regels ( + `<`, `>=`, `&&`, ) kan evalueren.
...
- Nieuw : mini-evaluator voor `checksum` (digit-vars d1..dn) en `formula` (var ). Gebruikt `text/template`-stijl parser of `go/parser` + eigen visitor — pure Go, geen deps.
- `validation.go`: `builtinValidators` map vervangen door:
  - regel-dispatch op : `checksum` → eval-engine; `formula` → eval-engine; `function` → kleine, expliciete map (, ) parallel aan FE.
...
- Nieuw  met de algemene types. Codegen-input bijwerken zodat de IDE deze als 1 bestand teruggeeft.
-  → leeg/weg.
-  → opheffen, IBAN naar gegevenstypen.
- `extra_datatype_registry.go` (Kleur, Duur, UrlHttps, GeoPunt) → samenvoegen in gegevenstypen.
-  (URL, Email, Telefoonnummer, GitAdres) → URL/Email/Telefoon naar gegevenstypen; GitAdres laten staan (CG-specifiek).
- Init-volgorde in  aanpassen.
...
- `TypeMeta` uitbreiden met `InhoudsVelden []VeldMeta` (, `JSONNaam`, `Datatype`, `IsPointer`, `Verplicht`).
- Codegen genereert de array vanuit V3Veld.
- `validation_walker.go` leest uit registry i.p.v. struct-tags. Reflectie alleen nog voor het ophalen van de waarde (kan met `reflect.ValueOf(rep).FieldByName(...)`, één keer per veld, of via een gegenereerde -methode per Representatie).
...
- **`Telefoonnummer` zit dubbel** als datatype én Go-alias (in `datatype_aliases.go` én ). Vergelijkbaar met `URL`. Bij hergroepering naar `gegevenstypen` opletten dat we niet duplicaten genereren in additieve codegen.
...
- **Validatie in  valideert ook `Afvoer`-representaties** — dat is meestal alleen een ID-payload. Misschien alleen Opvoer valideren? Te checken.
...
Akkoord. Ik begin met **R1** (generieke evaluator + hardcoded validators eruit) en **R2** (gegevenstypen-domein), en kijk naar NL API Strategie / RFC 9457 (Problem Details) voor het foutmelding-formaat. Documentatie volgt. Laat me eerst de huidige `validation.go`, , V3-structs, datatype-registries en (pad-)conventies snel inlezen, plus even oriënteren op NL API Strategie / RFC 9457.   Plan helder. Ik bouw het met **pure stdlib** (`go/parser` + `go/ast`) — geen externe dependency, en de modeleur schrijft Go-stijl-expressies die de FE óók al snapt. Plus RFC 9457 / NL API Strategie problem-details voor het foutformaat.

Stap 1: nieuwe expressie-evaluator. Stap 2: vervang  — hardcoded validators eruit, generieke regel-evaluator erin, plus RFC 9457 / NL API Strategie problem-details helper. Eerste build-check.  Build groen. Nu R2 — gegevenstypen-domein. Eerst de bestaande registries inspecteren om te zien wat er moet komen, plus het V3Datatype-veld voor  checken:      Ik voeg nu het gegevenstypen-domein toe + dedupe-helper + `geo_range` function. Eerst de plumbing-init-volgorde en : Nu het gegevenstypen-domein. Helper komt in het nieuwe bestand zodat de scope helder blijft.  Nu `geo_range` toevoegen aan de validatiefuncties + de init-volgorde aanpassen:  
```
...
```
R1.5 — handler-response naar Problem Details voor validatiefouten. Laten we de validatie-fout in  propageren met details, en in de Gin-adapter een  response sturen.  Ik breid `RegistreerError` uit met optionele Problem Details, zodat de Gin-adapter daar correct op kan reageren. 
```
...
```
Build + tests:   Voeg `"gegevenstypen"` toe als universeel-acceptabel domein in de export-scoring:   
```
...
Nu R3 + extra dekking. Laten we een korte evaluator-unit-test toevoegen en daarna documentatie:     Alle tests groen. Nu de documentatie bijwerken — gebruikers-preference is dat dit een vast onderdeel is. Ik maak een nieuwe `docs/validatie.md` (canonieke uitleg) en update  + `BACKLOG.md`.      Nu de status-update voor A.2 en het nieuwe documentje.  
```
...
- Hardgecodeerde `builtinValidators` map weg. Alle validatie loopt nu via  met types `checksum` / `formula` / `function`.
...
- Nieuwe `ProblemDetails` + . Bij blokkerende validatiefouten levert de Gin-adapter  met `invalidParams[]` (NL API DR-conform; UK Gov en NL bouwen beide op RFC 9457).
-  veld doorgegeven van core naar adapter.
...
- Init als laatste in .  accepteert `"gegevenstypen"` als universele bibliotheek.
...
**Fix** (handlers/registration_normalizer.go): als een kind een  of  heeft en de client stuurde een bare JSON-string, wordt die automatisch gewrapped als `{"datum": "..."}`. Beide vormen werken nu:
...
**Bug 1 (FK-injectie)**:  gebruikt  → voor hub-types retourneert dat `Rel_ID = 0`, terwijl  de entity-ID (501) verwacht. De 501 staat wel in .

**Bug 2 (meerdere hubs in één batch)**:  faalt bij meerdere actieve hubs. Maar de normalizer interleaves correct `[burgerschap_nl, aanvang_nl, burgerschap_pl, aanvang_pl]`, dus bij elke aanvang is de bijbehorende hub de **meest recent ingevoegde** (hoogste rel_id). Dat was het originele werkende patroon; de functie moet het hoogste rel_id teruggeven i.p.v. falen. Nu de twee fixes: 
```
...
De normalizer riep  aan. Voor een hub-type retourneert `GetID()` de  (op dat moment 0), terwijl het  van de aanvang de entity-ID (501) verwacht. Nu leest de normalizer de FK-waarde direct uit  van de parent-payload via de nieuwe helper .

**Bug 2 —  faalt bij meerdere actieve hubs**
...
2. Codegen — zodat toekomstige codegen de `schema:` tag ook doorzet in `_Input` Gevonden.  in  is ontworpen "zonder bun tags" maar laat ook de `schema:` tags weg. Die zijn juist nodig voor de validator. Twee fixes tegelijk: 
```
...
```
The  package isn't imported yet. Let me check: 
```
...
De validator wordt wél aangeroepen in , maar hij walkt over de `_Input` struct (dat is de daadwerkelijke  in de request). Die struct was codegen-gegenereerd door  in , en die helper liet de `schema:` tags weg ("zonder bun tags" was het commentaar, maar die gedachte sloeg ook de validatie-tags weg).

De `_Data` struct had `schema:"datatype:BSN"` wél, maar die wordt pas na de DB-insert geconstrueerd — te laat voor validatie.

**Fixes**

1. **`cmd/codegen/conventions.go`** —  propageert nu ook `schema:"datatype:..."` en `schema:"enum=..."` tags naar `_Input` structs. Toekomstige codegen-runs geven dus automatisch correcte validatietags.

2. **** —  heeft nu handmatig `schema:"datatype:BSN"` gekregen (zodat je nu al kunt testen zonder codegen te draaien).
...
I have all I need. Now I'll build everything in parallel: Now hook the init functions into : 
```

```
Build clean. Run the model tests: All tests pass. Now the Postman collection: Final build check:  All green. Now update the docs: 
```
...
-  — Hub + `_Data` structs met alle valideerbare velden ( tags)
-  — interface-implementaties + `GeefOnderliggendeGegevenselementen`
-  — `_Input` struct met alle schema-tags (wordt door de validatie-walker gebruikt)
-  — TypeMeta-entries (Entiteit, Hub, Data) + 
-  — leeg stub

**1 wijziging:**  — init-hooks toegevoegd na 
...
| `GeoPunt` Go-type | gecorrigeerd van `struct{Lat,Lng}` naar `string` — conform de bestaande implementatie in  |
...
```
Now add the three types to : 
```

```
The new types land in `gegevenstypenDatatypeRegistry` which is the canonical registry — no separate  entries needed (that file is for the "register"-domein extras like Kleur/Duur/GeoPunt/UrlHttps that predate this registry). Now the structs: 
```
...
```
Build clean, tests green. Build schoon, alle model-tests groen. Dit is wat er toegevoegd is:
...
|---|---|---|---|
| `AGBCode` |  | `agb-code` | `^\d{8}$` — exact 8 cijfers, geen checksum |
| `TIN` |  | `tin` | `^\d{9}$` + 11-proef (zelfde expressie als BSN/RSIN) |
| `EUBTWNummer` |  | `eu-vat` | `^[A-Z]{2}[A-Z0-9]{2,12}$` — 2-letter landcode + 2–12 alfanumeriek |
...
- model/gegevenstypen_modellen_input.go — dezelfde 3 velden als  in `_Input`
...
Laat me eerst de huidige staat van de bestanden controleren, plus de V3Datatype struct voor de API-URL-vraag.  Goed, nu zie ik de complete huidige staat. Laat me ook de `_ge_rel.go` en `_input.go` snel bekijken.  Goed overzicht. Nu heb ik alles wat ik nodig heb. Laat me de huidige `_ge_rel.go` slice rond de voertuigen-sectie zien, dan begin ik met implementatie.  
...
Goed — ik zie wat er aan de hand is. De vorige boodschap was per ongeluk, deze is wat je eigenlijk wilde delen: BAG-, BRK-, WOZ- en OID-nummers. Laat me ook de huidige staat van `_ge_rel.go` en `_input.go` even bekijken.  Nu heb ik genoeg context. Laat me de Postman-collection ook snel bekijken. Nu heb ik alle context. Laat me ook de happy path body bekijken om precies te weten hoe die uitgebreid moet worden. Nu heb ik alles. Ik zie ook dat de happy path in de Postman-collection de kenteken/paspoort/etc. velden van vorige sessie mist — die voeg ik meteen ook mee toe.
...
3. Skip voor nu: `KadastraleAanduiding` (composiet formaat, geen clean regex), `BGT-ID` (UUID-gebaseerd), `UOI` (nog in ontwikkeling), `Ligplaats-/StandplaatsID` (zelfde patroon als BAG, minder urgent) Nu doe ik alle Go-bestandswijzigingen in één batch: 
```
...
```
De eerste (happy path) is gelukt. Laat me de exacte tekst bij request 21→22 grens opzoeken.   Beide aanpassingen zijn toch doorgekomen (de error was misleidend). Laat me de sluiting van de Postman file verifiëren en daarna de build draaien. JSON is goed gesloten. Nu build + tests.  Build schoon (geen output = geen fouten).  Alles staat. Hier het overzicht:

---

**Vraag over API-URL's**: JA, slim om op te nemen. Opgelost via nieuw -veld in `V3Datatype` (model/v3_format.go). Machine-leesbaar, `omitempty` zodat bestaande entries niet breken, en later te tonen als "meer info"-link in de frontend/schema-API.
...
Nu heb ik het beeld. Ik lees de volledige  zodat ik precies weet wat er al staat en wat overlapt met de backlog: 
...
Ik lees eerst de huidige staat van de relevante bestanden:  Nu lees ik de registry en de validatie-functies op hun 
