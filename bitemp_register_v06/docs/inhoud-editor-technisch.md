...
6. **Secondaire entiteit-ID bij relaties**: Relaties hebben een `secondaireEntiteitIDKolom` (bijv. `locatie_id` bij Bereikbaarheid).
   ...
   **Nieuwe-entiteitpagina (`/t/:typePad/nieuw`, sept. 2026)**: in `NieuwEntiteitPagina` → `ActionGroupedSections` wordt de secundaire id-kolom van een relatie nu getoond met de naam van de doel-entiteit (bv. **gemeente** i.p.v. `gemeente_id`; de kolomnaam staat in de tooltip, de payload blijft `gemeente_id`). `bouwGroepOptiesVanTypeMeta` levert daarvoor `secondaireLabel` en `secondaireRefType`. Is de doel-entiteit een `referentielijst_item` (bv. Gemeente, 343 items), dan rendert `ActionFieldControl` een `RefCombobox` met server-side zoeken — de oude `<select>` via `/full/{padnaam}?size=200` werd door de server op 100 rijen afgekapt. Voor overige doel-entiteiten pagineert de dropdown nu over `/full/` (100 per pagina, max. 20 pagina's).

   `RefCombobox` (grote lijsten, > 30 items): openen via ▼ zonder zoekterm laadt nu de eerste 50 opties (voorheen "Geen resultaten"); `loading` gaat direct aan tijdens de debounce; verouderde zoekantwoorden (bv. voor "Z" na "Zo") worden via een volgnummer genegeerd; minimale breedte zodat het veld in smalle flex-rijen niet tot ~44px inklapt.

   `GET /api/viz/reflijst/{typenaam}/opties` selecteert nu alleen `_data`-rijen onder een **actieve hub** (`EXISTS … h.afvoer IS NULL`). Aanleiding: een open data-rij onder een afgevoerde hub (Gemeente 1993 na registratie 942: `opvoer gemeentegegevens` met expliciete `rel_id: 1` op een enkelvoudig hub-GE sloot hub 1, maakte hub 2 aan, maar hing data v2 nog onder hub 1) gaf een dubbele optie.
