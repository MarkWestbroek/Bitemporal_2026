...
| E26| Lock-status persistent maken in V3 JSON / diagram-store, zodat 🔒 een herstart van de editor of een her-import overleeft (nu alleen runtime React Flow node-state). Voorgestelde plek: `data.layoutLocked` op de `V3Entiteit/Relatie` editor-layout-sectie en op `DiagramNode` in de IDE-store. | nieuw |
