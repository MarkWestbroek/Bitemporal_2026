`# Backlog â€” Bitemporeel Register v06
...
> **Doel**: Ã©Ã©n overzicht van alle openstaande features, ideeÃ«n, verbeterplannen en toekomstige stappen.

---

## 0. Acute issues â€” IDE / editor (2026-04-27)
...
### 0.2 IDE mermaid-import is "kapot" â€” âœ… OPGELOST (2026-04-27)
- Was: alleen klassen kwamen binnen, geen velden, geen edges, geen default diagram.
- Root cause: IDE-pad ging `rawUMLNaarEditor` â†’ `editorNaarV3Model` â†’ `v3ModelNaarStore`. `editorNaarV3Model` (`src/umleditor/metamodel/types.js:669`) ondersteunt geen velden op entiteit-niveau (V3 EntiteitModel kent alleen GE's) en geen directe entiteitâ†’entiteit edges, dus die werden stilletjes weggegooid.
- Oplossing â€” "rawUML"-pad zonder V3-tussenstap, lossless:
  - Nieuwe `rawEditorNaarStore(graaf, opts)` in `src/store/adapters.js`: 1-op-1 mapping van editor-nodes/edges naar IDE-store; behoudt velden op entiteiten en directe entâ†’ent edges; Ã©Ã©n overzicht-diagram met posities uit de bron.
  - `ImportDialog.jsx`: textuele UML-imports worden ingepakt als `_format: "raw-editor"` (niet meer als V3-envelope).
  - `pages/IdePage.jsx` `handleImportResult`: extra case `"raw-editor"` â†’ `rawEditorNaarStore`.
  - `DetailsPanel.jsx`: voor entiteit met losse velden tonen we `VeldenEditor` met een waarschuwingsbanner ("gebruik B6 om te splitsen"). Bewerkbaar â€” kleine opschoning vÃ³Ã³r splitsen blijft mogelijk.
  - Nieuwe `valideerVoorV3(state)` in `adapters.js`: vlagt entiteit met velden (V3-001) en directe entâ†’ent edges (V3-002). Pas bij export/build af te dwingen.
...
### 0.3 Orphan-testbestanden 01â€“04 falen bij IDE-import â€” âœ… OPGELOST (2026-04-27)
...
### 0.4 B5/B6/B7 alleen in IDE, niet in editor-v2 â€” âœ… OPGELOST (2026-04-28)
...
- Rechtsklik op enkelvoudige entiteit â†’ nieuw "Refactor"-menu (menuType `"refactor"`) met:
  - **âœ‚ï¸ Splits velden naar GE's...** (B6): opent dialoog met checkboxes per veld â†’ maakt per veld een nieuw GE-node + compositie-edge; verwijdert velden uit de entiteit.
  - **ðŸ”„ Cast entiteit naar GE...** (B5): opent dialoog met dropdown voor de parent-entiteit â†’ converteert node-type naar `gegevenselement`; verwijdert losstaande entiteit-edges; voegt parent-edge toe.
...
- Undo-integratie: beide acties pushen een canvas-snapshot â†’ `Ctrl+Z` werkt.
...
### 0.5 IDE B5 Cast-naar-GE: window.prompt â†’ modal dropdown â€” âœ… OPGELOST (2026-04-28)
...
- Opgelost: inline modal met `<select>` dropdown, analoog aan de B5-modal in editor-v2 en de B6-splits-modal in de IDE.
  ...
  - `handleCastDialoogBevestigen` roept `castEntiteitNaarGE` uit `transformations.js` aan, verwerkt warnings via `window.confirm`, past lokale RF-state aan (node-type â†’ `gegevenselement`), en roept `passToePatch`.
  - Import `promptCastNaarGE` verwijderd; `castEntiteitNaarGE` toegevoegd aan import uit `./transformations`.
  - Theming via CSS-variabelen (`--ide-panel-bg`, `--ide-input-bg`, etc.) â€” werkt in zowel light als dark mode.
- Bestanden: `DiagramCanvas.jsx`.

### 0.6 Node-tekstleesbaarheid in dark mode â€” âœ… OPGELOST (2026-04-28)
...
- Opgelost: dark-mode CSS-overrides voor node-tekstkleuren zetten tekst altijd op donker:
  ...
  - `.veld-type`: `#475569` (was `#64748b` â€” zelfde waarde maar nu bewust als "donker op lichte achtergrond").
  - `.node-veld.enum-waarde`: `#92400e` donkere amber (was `#fcd34d` geel â€” onleesbaar op lichte achtergrond).
  - `.node-veld.leeg`: `#64748b` (was `#475569` â€” iets lichter maar nog steeds leesbaar op wit/lichtgroen).
...
### 0.7 Project Browser â€” domein hernoemen + verwijderen
- Hernoemen: prompt voor nieuwe naam â†’ controle of doelnaam al bestaat â†’ alle elementen + diagrammen met `domein === oud` updaten.
- Verwijderen: confirm-dialoog met telling van te raken elementen + diagrammen â†’ cascade delete of verplaatsen naar `(geen domein)`.
...
#### Tab-naam sync bij hernoemd diagram â€” âœ… OPGELOST
...
#### Rebuild-validatie: domein-scope + leesbare foutmeldingen â€” âœ… OPGELOST
- Was: `validateV3Model()` valideerde altijd het volledige model, ook wanneer in de publiceer/rebuild-dialoog slechts Ã©Ã©n domein geselecteerd was. Foutmeldingen gebruikten index-context (`entiteiten[17].relaties[3]`) â€” onleesbaar.
- Opgelost:
...
#### A3: afgeleide velden op relatie triggeren ASOC-expansie â€” âœ… OPGELOST
...
- Opgelost: trigger-formule telt nu zowel `velden` als `afgeleideVelden` mee â€” Ã©Ã©n regel wijziging in de `useEffect`. De canonieke `relatieVorm()`-helper in `shared/asoc.js` (al correct) blijft single source of truth voor de uiteindelijke vorm.
- Bestanden: `src/ide/DiagramCanvas.jsx`.

#### B6 + 0.7: diagram + domein verwijderen via PB-rechtsklik â€” âœ… OPGELOST
...
- Opgelost:
  - `BrowserContextMenu.jsx`: twee nieuwe menu-items "ðŸ—‘ï¸ Verwijder diagramâ€¦" (op `diagram`-nodes) en "ðŸ—‘ï¸ Verwijder domeinâ€¦" (op `domain`-nodes).
...
#### Visuele e2e-tester (Playwright) â€” âœ… SKELET TOEGEVOEGD (2026-04-30)
- Doel: agent (en mens) kan visueel UI-flows verifiÃ«ren met screenshots, traces en headless/UI-runs.
- Toegevoegd:
  ...
  - `web/vite/tests/e2e/01-ide-laadt.spec.js`: smoke â€” `/ide/` laadt en window-hook is beschikbaar.
  - `web/vite/tests/e2e/02-verwijder-diagram-context-menu.spec.js`: rechtsklik op diagram â†’ "Verwijder diagram" zichtbaar â†’ annuleren â†’ diagram blijft staan.
  - `web/vite/tests/e2e/helpers/model.js`: `injectMinimaalModel(page)` â€” vult store met 2 entiteiten + 1 diagram via `window.__useModelStore`.
...
Inventarisatie van een batch vragen en bugs over editor-v2 Ã©n IDE. GeÂ­ordend op grootte; onderlinge afhankelijkheden gemarkeerd.

**A. Bugs (kleinâ€“middelgroot)**

1. **ASOC edge-labels worden niet getoond.** Eerder werkte de spiegel-conventie: het label *aan de ankerkant* van de Aâ†’anker edge is feitelijk het label *bij B*, en omgekeerd. Daarbij hoort: alleen de relatieÂ­eigenschappen (multipliciteit bij A, multipliciteit bij B, naamLabelHeen, naamLabelTerug) bewerken op de relatie-node; edges zijn slechts visualisatie. Status: regressie â€” ergens gesneuveld.
2. **Lege veld-compartiment in entiteit-nodes** geeft een dubbele lijn. Wanneer een entiteit gÃ©Ã©n directe velden heeft (normale situatie in V3) moet het lege compartiment verborgen worden zodat er nog Ã©Ã©n scheidingsÂ­lijn overblijft.
3. **Afgeleid veld toevoegen aan een relatie expandeert deze niet naar associatieÂ­klasse.** âœ… OPGELOST (2026-04-29) â€” trigger in `DiagramCanvas.jsx` telt nu ook `afgeleideVelden` mee.
4. **V3-import in IDE bouwt diagram niet op.** Bij import van een V3 JSON die uit editor-v2 komt (mÃ©t layout) wordt in de IDE geen diagram gereconstrueerd. Doel: editor-v2 als schetsblok kunnen gebruiken en naar IDE exporteren *inclusief* layout (posities, edges, anker-posities).

**B. Features (middelgroot)**

5. **Verplaatsbare edge-labels.** Labels (rolnamen, multipliciteiten, anker-labels) moeten met de muis verplaatst kunnen worden, met persistente offset. PersistentieÂ­paden:
...
6. **Diagram verwijderen via rechtsklik in IDE Project Browser.** âœ… OPGELOST (2026-04-29) â€” confirm met telling, sluit ook open tabs.
7. **Domein verwijderen via rechtsklik in IDE Project Browser.** âœ… OPGELOST (2026-04-29) â€” tweetraps-confirm met cascade-keuze; "(geen domein)" geweigerd.
...
9. **Afgeleide associaties / GE's / RELs.** Analoog aan afgeleide velden: een relatie of GE die compleet door regels berekend wordt (CEL-expressie over andere modelÂ­elementen). Vraagstukken om uit te werken:
   - Berekening: live (FE) vs. gematerialiseerd (BE) â€” instelling per item
...
10. **Taal als fundamenteel aspect (i18n / meertaligheid).** DiscussieÂ­stuk in `docs/2026-04-29 taal aspect.md`: moet meertaligheid een **core-aspect** worden van representaties (Ã  la `materieel` â†’ optioneel `aanvang/einde`; analoog `talig` â†’ optioneel `taal`-veld op _Data, met multipliciteit "enkelvoudig per taal"), of blijft het een per-register modelleringsÂ­keuze (zoals nu de `Taalvariant`-cluster in het kennis-domein)? Onderzoek nodig naar:
    - Bestaande patronen / standaarden in CMS-, API- en register-wereld (Drupal/Magnolia/Adobe AEM, OData, JSON-LD `@language`, IETF BCP 47, ICU, INSPIRE, ISAÂ²)
    - Impact op MetaRegistry, codegen, schema-API, querystring (`?taal=nl`), GraphQL en frontend-rendering
    - Multipliciteit-semantiek: "enkelvoudig in tijd Ã©n in taal" als gecombineerde constraint
    - Migratie-pad voor bestaande registers

---

### 0.9 V3 uitwisseling: A4-rev (diagrammen), B5 (label-offsets), C8 (notes/constraints) â€” datalaag IMPLEMENTED (2026-05)
...
- **A4-rev (V3 â†” IDE diagram-roundtrip):**
...
- **B5 (verplaatsbare edge-labels â€” datalaag + UI):**
  - Go: `V3LabelOffsets { heen?, terug? }` met `V3Offset { x, y }` per `V3DiagramEdge`.
  - IDE-store: edge `data.labelOffsets.{heen,terug}.{x,y}`; geÃ¼pdatet via bestaande `updateDiagramEdge(diagramId, edgeId, patch)`.
  - Adapter: serialiseert offsets alleen als ze gezet zijn (â‰  {0,0}).
  - UI: `MetamodelEdge.jsx` rendert nu twee extra labels (`naamLabelHeen` en `naamLabelTerug`) met UML-driehoekjes (â–¶ / â—€), schaal-bewust draggable (zoom-correct), en met dubbelklik-reset naar default-positie. Werkt op compositie- Ã©n associatie-edges.
  - Test: `v3_b5_c8_roundtrip.test.js` valideert dat offsets behouden blijven door storeNaarV3Model + v3ModelNaarStore.
- **C8 (notes en constraints â€” datalaag):**
...
**Open follow-ups:**
- B5: cross-platform â€” editor-v2 leest/schrijft `labelOffsets` nog niet; idem MetaRegistry/DB-publicatie.
...
## 1. README.md â€” TODO-sectie (lijnnummers 811+)

### API & Backend

```
05  Log Ã lle requests en responses? (hoe?)
...
Plan: Ã©Ã©n gedeelde engine `RegistreerCore` met drie schrijflagen.
Sessieplan: zie `/memories/session/plan.md`.

- âœ… **Fase 1 (geneste full-payload, 2026-04-28).** Een `WijzigingRequest.opvoer/afvoer` mag een geneste boom bevatten (zelfde shape als `GET /full/{padnaam}/:id`-response). De server splitst die in platte wijzigingen, Ã©Ã©n per representatie. Audit-granulariteit blijft fijnmazig; de originele geneste payload wordt letterlijk in `Registratie.RequestBody` bewaard. Implementatie: [handlers/registration_normalizer.go](handlers/registration_normalizer.go), `RepresentatiePlusNaam.RawPayload` in [model/REST request models.go](model/REST%20request%20models.go), wiring + raw-body-capture in [handlers/registration_handlers.go](handlers/registration_handlers.go). Tests: [handlers/registration_normalizer_test.go](handlers/registration_normalizer_test.go) + [handlers/registration_normalizer_nploc_test.go](handlers/registration_normalizer_nploc_test.go).
- âœ… **FK-propagatie (2026-04-28, cross-cutting).** Client hoeft de parent-FK (`{ent}_id`) **nooit** mee te sturen in geneste children â€” de normalizer injecteert die automatisch via `injecteerParentFK` in `registration_normalizer.go`. Geldt voor Fasen 1/2/3. E2E geverifieerd: POST NatuurlijkPersoon zonder FK in children â†’ response bevat `natuurlijkpersoon_id` in alle child-records.
- âœ… **Padnaam-fallback in flat format (2026-04-29).** `GetByVeldnaamMetPayload` valt nu terug op `Padnaam` (meervoud) als de outer key niet als `Veldnaam` (enkelvoud) bekend is. Hierdoor werkt het klassieke platte format ook met `"persoonsidentificaties"`/`"namen"` (consistent met geneste format). Implementatie + nieuwe `GetAllByPadnaam` in [model/metaregistry_plumbing.go](model/metaregistry_plumbing.go); 4 subtests in [model/metamodel_test.go](model/metamodel_test.go).
- âœ… **Input-structs gesaneerd (2026-04-29).** `_Input`-structs van materiÃ«le GE's/relaties (`NatuurlijkPersoon_Burgerschap_Input`, `Bereikbaarheid_Input`, en CG-types `Initiatief_Planning/Product/Bijdrage/Beoordeling_Input`) hadden directe `Aanvang *Date`/`Einde *Date` velden, terwijl de MetaRegistry deze al als onderliggende GE's (`*_Aanvang`/`*_Einde`) definieert. Resultaat: payloads met `"aanvang": {"datum": "..."}` faalden op unmarshal (object â†’ string). Velden verwijderd; de normalizer splitst `aanvang`/`einde` nu correct uit als aparte wijzigingen â€” conform Hub+_Data ontwerp.
- âœ… **Fase 0 (refactor, 2026-04-29).** Pure engine `RegistreerCore(ctx, db, req, audit) (RegistreerResult, *RegistreerError)` geÃ«xtraheerd in [handlers/registration_core.go](handlers/registration_core.go). Alle helpers in [handlers/registration_helpers_generiek.go](handlers/registration_helpers_generiek.go) gebruiken nu `context.Context` i.p.v. `*gin.Context` (mechanische refactor; helpers gebruikten `c` enkel voor `c.Request.Context()`). De ONGEDAANMAKING-tak leeft in `verwerkOngedaanmaking`. `RegistreerMetNieuweAanpak` in [handlers/registration_handlers.go](handlers/registration_handlers.go) is nu een dunne Gin-adapter (~75 regels, was ~513): rawBody lezen â†’ normaliseren â†’ `AuditMeta` bouwen â†’ Core aanroepen â†’ JSON-response. Response-shape (incl. `registratieId`-alias) ongewijzigd. Tests: 4 nieuwe core-tests in [handlers/registration_core_test.go](handlers/registration_core_test.go) (happy path, BeginTx-fout, Insert-fout/rollback, audit-velden); bestaande gin-handler-tests blijven groen.
- ðŸŸ¡ **Fase 2 (REST/CRUD per padnaam, deels af).**
  - âœ… **DELETE per padnaam (2026-04-29).** `MakeDeleteEntityByMetaHandler` in [handlers/crud_handlers.go](handlers/crud_handlers.go) laadt het record, bouwt een `RegistreerRequest` met Ã©Ã©n `Afvoer`-wijziging en delegeert naar `RegistreerCore`. Audit-trail + transactiegedrag identiek aan POST `/registratie/`. Geregistreerd in [routes/addroutes_helper.go](routes/addroutes_helper.go); OpenAPI uitgebreid in [handlers/openapi_generator.go](handlers/openapi_generator.go) (`delete`-operatie op `{padnaam}/{id}`). PFK-types worden expliciet afgewezen met 400 (composite key niet adresseerbaar via Ã©Ã©n URL-id) â€” gebruik daar POST `/registratie/`. Tests: 3 nieuwe scenario's in [handlers/crud_handlers_test.go](handlers/crud_handlers_test.go) (PFK afwijzing, 404, happy-path delegatie).
  - âœ… **PATCH /full/{padnaam}/:id (2026-04-29).** JSON Merge Patch (RFC 7396) op onderliggende GE's/RELs. Pure wijziging-builder in [handlers/wijziging_builder.go](handlers/wijziging_builder.go) (`BouwWijzigingen(input) â†’ []WijzigingRequest + meldingen[]`); handler `MakePatchFullEntityByMetaHandler` in [handlers/crud_handlers.go](handlers/crud_handlers.go) delegeert naar `RegistreerCore`. Hybride wrapper-detectie (variant A mÃ©t, B zonder ENT-wrapper). `?modus=registratie|correctie` (default `registratie`); correctie vereist `rel_id` per GE/REL. ENT-velden zelf zijn niet patchable (400). Response bevat `meldingen[]` met niet-fatale waarschuwingen (genegeerde `rel_id` in registratie, no-op items in correctie). Route en OpenAPI bijgewerkt. **Parent-context disambiguatie**: bouwt `RepresentatiePlusNaam` direct vanuit `og.Doeltype` i.p.v. globale `GetByVeldnaamMetPayload` â€” lost veldnaam-collisions op (bv. `"naam"` = `NatuurlijkPersoon_Naam` Ã©n `ApiStandaard_Naam`). Tests: 12 scenario's in [handlers/wijziging_builder_test.go](handlers/wijziging_builder_test.go). Foutcodes en design gedocumenteerd in [docs/REST_CRUD.md](docs/REST_CRUD.md).
  - ðŸŸ¡ **ETag / If-Match (concurrency, optioneel, follow-up).** Ontwerp gedocumenteerd in [docs/REST_CRUD.md](docs/REST_CRUD.md) (sectie "Concurrency"). Niet in code in deze iteratie â€” implementatie vergt een query die de hoogste `registratie_id` voor een entiteit bepaalt over meerdere onderliggende GE-tabellen.
- ï¿½ **Fase 3A (GraphQL: directe core-calls, 2026-04-30).** De HTTP-roundtrip in `dynql/mutation_resolvers.go` is verwijderd. Mutations `registreer` / `corrigeer` / `maak_ongedaan` roepen nieuwe `handlers.RegistreerJSONCore` direct aan (Ã©Ã©n audit/transactie-pad). Pure cores `WijzigEntiteitCore` en `VoerEntiteitAfCore` geÃ«xtraheerd; PATCH/DELETE-handlers zijn nu dunne Gin-adapters.
- ðŸŸ¢ **Fase 3B-light (per-ENT typed mutations, 2026-04-30).** Voor elk entiteit-type registreert [dynql/typed_mutations.go](dynql/typed_mutations.go) drie mutations: `wijzig<Typenaam>(id, patch: JSON!)`, `corrigeer<Typenaam>(id, patch: JSON!)`, `voer<Typenaam>Af(id)`. Modus zit in de naam (geen `modus`-arg). Hergebruikt PATCH/DELETE-cores. Tests in [dynql/typed_mutations_test.go](dynql/typed_mutations_test.go).
- ï¿½ **Fase 3B-full (typed `<Typenaam>PatchInput`, 2026-05-xx).** Het `JSON!` scalar in `wijzig<X>`/`corrigeer<X>` is vervangen door dynamisch gebouwde `*graphql.InputObject` types. Nieuw bestand [dynql/input_type_builder.go](dynql/input_type_builder.go): `BuildPatchInputTypes()` bouwt bij startup `<GETypenaam>Input` (via reflectie op `_Data`-structs) en `<Typenaam>PatchInput` (via `OnderliggendeGegevenselementen`). Materieel: `aanvang`/`einde` sub-inputs (`PlumbingDatumInput`). Relaties: secundaire FK opgenomen. Tests in [dynql/input_type_builder_test.go](dynql/input_type_builder_test.go) (8 cases). Resolvers onveranderd â€” graphql-go deserialiseert InputObject naar `map[string]interface{}`.
...
- ðŸŸ¡ **Domein/parent-context filter op `GetByVeldnaamMetPayload` (2026-04-29).** De huidige globale lookup faalt bij veldnaam-collisions tussen domeinen (`"naam"` = `NatuurlijkPersoon_Naam` Ã©n `ApiStandaard_Naam`); de payload-key disambiguatie (`<ent>_id`) helpt alleen als de juiste FK-kolom in de payload zit. Voorgestelde uitbreidingen:
  - `GetByVeldnaamBijOuder(ouderMeta, jsonRolnaam)` â€” kijkt alleen in `ouderMeta.OnderliggendeGegevenselementen`. Sterkste vorm; al gebruikt in PATCH-builder via `og.Doeltype`.
  - `GetByVeldnaamWithinDomein(veldnaam, domein, payloadKeys)` â€” beperkt tot Ã©Ã©n domein. Nuttig in `RepresentatiePlusNaam.UnmarshalJSON` als parent al bekend is (POST `/registratie/` met geneste payload, normalizer).
...
**Validatie-walker** (`model/validation_walker.go`): reflecteert over exported fields op zoek naar `schema:"datatype:X"` tags; voor string-kinded velden wordt `ValideerWaarde(datatypeNaam, s, veldPad)` aangeroepen. Gecombineerd resultaat = Ã©Ã©n `ValidationResult` met alle fouten. Aanroeppad: `ValideerRepresentatie(rep)` â†’ walker â†’ `ValideerWaarde` per veld.

- âœ… **R1: BSN 11-proef + IBAN mod-97 + NLPostcode regex (sessie 2026-04).** `ValideerWaarde` dispatcht op datatypeNaam; eigen implementaties voor BSN (11-proef), IBAN (mod-97), NLPostcode, Emailadres, URL, Kleur, GeoPunt enz. in `gegevenstypen_datatype_registry.go`. Tests in `model/validation_test.go`.
- âœ… **R2: Validatie-walker + `ValideerRepresentatie` (sessie 2026-04).** Walker reflecteert `_Input`-structs; groepeert foutmeldingen per veldpad; retourneert HTTP 422 bij eerste ongeldige request in `registration_core.go`. `ValideerRepresentaties([]Representatie) error` als batch-helper. Tests: `TestValideerRepresentatie_Walker` + `TestValideerRepresentatie_InputStructBSN` in `model/validation_test.go`.
- âœ… **Normalizer-regressions gefixed (sessie 2026-04).** Twee bugs in batch-registraties:
  1. FK-injectie (`injecteerParentFK`) gebruikte `GetID()` i.p.v. `leesIntUitRawMap(rawMap, childMeta.EntiteitIDKolom)` â€” nieuwe helper in `handlers/registration_normalizer.go`.
...
- âœ… **`_Input`-structs missen `schema:"datatype:..."` tag (sessie 2026-04).** Walker werkt op de `_Input`-struct (die gematerialiseerd wordt in `registration_core.go`), niet op `_Data`. Oplossing: (1) tag handmatig toegevoegd aan `NatuurlijkPersoon_Persoonsidentificatie_Input.Bsn`; (2) `inputContentField` in `cmd/codegen/conventions.go` propageert nu `schema:`-tags (datatype, enum) naar gegenereerde `_Input`-structs. Test: `TestValideerRepresentatie_InputStructBSN`.
- âœ… **`TestEntiteitGegevenstypen` + Postman collection (sessie 2026-04).** Handmatig aangemaakt testmodel in het gegevenstypen-domein:
  ...
  - Entiteit `TestEntiteitGegevenstypen` + Hub GE `TestEntiteitGegevenstypen_TestGEGegevenstypen` met Ã©Ã©n `_Data`-struct die elk string-gebaseerd valideerbaar gegevenstype (BSN, KvK, NLPostcode, IBAN, Emailadres, Telefoonnummer, URL, UrlHttps, Kleur, GeoPunt, KorteTekst, AN40, AN200, Datum, Duur) als apart veld met `schema:"datatype:X"` tag bevat.
  - Init-functies gekoppeld in `model/metaregistry_plumbing.go` (na `initGegevenstypenDatatypeRegistry`).
  - Postman-collectie: `postman/validatietest-gegevenstypen.postman_collection.json` met 13 requests: entity aanmaken, happy path, Ã©Ã©n invalid request per datatype (requests 2â€“11), en multi-fout test (request 12). Verwacht HTTP 422 bij elke invalid request.
...
30  UML model versies
    - ~~delta tussen een nieuwe en de huidige bepalen~~ âœ… (schemadiff + IDE Delta-knop)
    - ~~impact van de delta bepalen (breaking of niet)~~ âœ… (ernst-classificatie in schemadiff)
...
```
31  UML editor
    - meerdere canvassen, per domein Ã©Ã©n (of naar keuze)
    - afhankelijkheid kunnen instellen
    - overerving zelf kunnen tekenen âœ…
    - ~~alignen~~ âœ… (context menu align-acties)
    - relatie-visualisatie:
      - ~~met velden: associatieklasse~~ âœ… (ASOC-patroon: anker + 3 edges)
        - ~~probleem: de lijnen tussen A en REL en REL en B zijn geen relaties, maar alleen maar de link~~ âœ…
          - ~~ze hebben een richting: hoe visualiseren?~~ âœ… (directioneel checkbox + pijl op ankerâ†’B)
      - ~~zonder: alleen een lijn met een label "relatie"~~ âœ… (collapsed badge)
      - labels bij rollen verplaatsbaar (hoe in V3 en metareg?)
    - ~~V3 import ASOC alleen bij velden~~ âœ…
    - ~~reverse ASOC behoudt directioneel~~ âœ…
    - ~~default bestandsnaam opslaan = versie~~ âœ…
    - ~~normaliseer alle relaties (toolbar + context menu)~~ âœ…
    - ~~snap alle elementen naar grid (toolbar + context menu)~~ âœ…
    - ~~auto-layout: logische ordening van alle REPs (ENT centraal, GE's eronder/ring, RELs tussen ENTs, per domein gegroepeerd) â€” toolbar `ðŸŽ¯ Auto-layout` + rechtsklik op canvas~~ âœ…
    - ~~kopiÃ«ren/plakken van nodes tussen diagrammen (Ctrl+C/V + rechtsklik)~~ âœ…
    - ~~Shift+drag entiteit vanuit PB = ENT + alle onderliggende GE's/relaties~~ âœ…
...
```
35  IDE
    - ~~multiselect in PB~~ âœ… (Ctrl+click toggle, Shift+click range)
    - ~~drag and drop GE en ENT is er al: relaties moeten getekend~~ âœ… (auto-add owner + doel entiteiten)
      - ~~niet altijd (reproduce)~~ âœ…
    - ~~drag and drop - complete (shift?): ENT + alle GE's (en RELs) mee?~~ âœ… (Shift+drag entiteit bevat alle onderliggende + doel-entiteiten worden auto-toegevoegd)
    - ~~Ctrl+click multi-select + multi-drop~~ âœ…
    - auto-order
    - ~~dubbelklik op edge: straighten~~ âœ… (berekenKortsteHandles)
    - undo / redo doet het niet âœ… 
    - edge types: ~~compositie~~âœ…, ~~overerving~~âœ…, ~~associatieklasse~~âœ… (!)
    - ~~diagram of any element rename in PB~~ âœ… (tab-naam sync via `renameDiagramTab` + PB-prop + F2-handler â€” 2026-04-29)
    - domeinkleur instellen, uberhaupt properties van domein instellen mogelijk âœ…
      - welke properties allemaal?
    - layout bar verplaatsbaar âœ…
    - any bar
    ...
    - velden (attr) ook omschrijving
      - meer dan dat: [0..*] enz
    - ~~normaliseer alle relaties~~ âœ… (gedeeld via MetamodelEditor)
    - ~~snap naar grid~~ âœ… (gedeeld via MetamodelEditor)
```
...
- Een _data element op een GE (todo, punt 2 eerst)  (lijn 745 â€” DONE in DB)
```

---

## 2. docs/TODO.md â€” API Logging
...
## 3. docs/IDE.md â€” Toekomstige fasen

### Fase 3 (Multi-diagram) â€” onvoltooide items

```
- Meerdere diagram-tabs naast elkaar
- Node toevoegen aan diagram â‰  element aanmaken
- Node verwijderen van diagram â‰  element verwijderen
...
```
- Lokale persistentie eerst: Zustand persist â†’ localStorage. Database-sync en multi-user is een latere fase.
```

---

## 4. docs/DEVLOOP.md â€” Geen expliciete TODO's

Documentatie is actueel. Geen openstaande items gevonden.

---

## 5. plans/2026-03-29 Forms plan 01.md â€” Content Editor Plan
...
## 6. plans/2026-03-29 Forms plan 02.md â€” Content Editor Plan (uitgebreid)

### Iteratie 2 â€” Custom Formulieren (nog niet geÃ¯mplementeerd)

```
15. Formulierdefinitie-schema â€” JSON-formaat dat layout en veldgroepering beschrijft
16. <CustomFormulier> â€” Renderer die formulierdefinitie + schema-API data combineert
17. Conditionele zichtbaarheid â€” Velden/secties tonen/verbergen op basis van andere veldwaarden
18. Formulierdefinities opslaan â€” In database of als JSON, gekoppeld aan entiteittype
```
...
- Formele/materiÃ«le tijdreis in de editor (peil-/tijdstipkiezer)
- Audit-trail weergave per record
- Role-based access control op veld-/formulierniveau
- Drag-and-drop formulier-builder (visueel, Ã  la form.io)
```
...
## 8. plans/2026-03-31 dynamic graphql plan â€” Dynamische GraphQL

Volledig plan voor vervanging van de huidige gqlgen-implementatie:

```
Fase 1: Infrastructuur â€” dependency graphql-go/graphql, directory dynql/ met 7 bestanden
Fase 2: Output types bouwen â€” per MetaRegistry entry met reflectie
Fase 3: Query resolvers â€” full entity, lijst, registraties, formeel tijdreizen
Fase 4: Mutation resolvers â€” registreer mutation â†’ hergebruik registratie-flow
Fase 5: Integratie â€” vervang gqlgen, verwijder graph/ (~10.300 regels)
Fase 6: Verfijning â€” field selection, enum types, afgeleide velden, referentielijsten
...
## 9. ontwerpgedachten/2026-04-03 domeinen â€” Domeinbeheer
...
- GE met domein Y niet koppelbaar aan ENT uit domein X â†’ melden, vragen om verplaatsing
- Validatie vÃ³Ã³r publish/rebuild:
...
## 10. UML_EDITOR_INTEGRATIE.md â€” Editor toekomst

### uml-editor/README.md â€” Toekomstige mogelijkheden

```
- MetaRegistry-generatie: editor-output omzetten naar Go-code (MetaRegistry entries + struct definities)
- Validatie: controle op naamconventies, verplichte velden, referentiÃ«le integriteit
```

---

## 11. GRAPHQL.md â€” Next Steps

```
1. Implement Task Resolvers â€” Start with the existing Task model since you already have it
2. Implement Entity Resolvers â€” Use Entity A/B as templates
3. Add Filtering â€” Enhance queries with filter inputs
4. Add Sorting â€” Support ordering results
5. Batching â€” Use DataLoader for N+1 query prevention
6. Authorization â€” Add middleware for security
```
...
## 12. afgeleide-velden.md â€” Toekomstige doorontwikkeling
...
## 13. CEL-evaluatie-js.md â€” Frontend CEL
...
## 14. materiele_tijd.md â€” Toekomstige uitbreidingen

```
- MateriÃ«le tijdreizen: queryparameter geldig_op=2023-06-15 om de toestand op een
  materieel peiltijdstip te bevragen (combinatie met formeel peiltijdstip â†’ volledige bitemporaliteit).
...
- MateriÃ«le validatie: controle dat einde >= aanvang, en dat periodes niet overlappen
  bij een nieuw opgevoerde aanvang/einde.
```

---

## 15. RELEASE.md â€” Runtime fix
...
// wijziging(en) in Ã©Ã©n endpoint verwerken
```

---

## 17. model/ontwerpkeuzen.md â€” Delta-analyse âœ… GEÃMPLEMENTEERD

**Status**: volledig geÃ¯mplementeerd in `schemadiff/` package + `cmd/schemadiff/` CLI + `--diff`/`--diff-only` in codegen + IDE integratie via `POST /admin/diff/:password`.
...
## 18. docs/API-standaarden-analyse.md â€” Aanbevelingen
...
## 19. docs/codegen_analyse_roundtrip.md â€” Volgende stappen
...
4. Roundtrip-test: exporteer np-loc model â†’ genereer â†’ diff met hand-geschreven code
5. Itereer tot diff leeg is (of alleen verwachte volgorde-verschillen bevat)
```

---

## 20. docs/overerving-analyse.md â€” Volgende stappen
...
6. Frontend: geÃ«rfde velden tonen in formulieren
```

---

## 21. docs/inhoud-editor-technisch.md â€” Bekende aandachtspunten
...
## 22. docs/inhoud-editor-handleiding.md â€” FAQ-items
...
## 23. Referentielijsten.md â€” Openstaande items

```
- Fase B t/m H: Gepland maar status uit plan zegt âœ… Compleet (check: de Referentielijsten.md
  zelf toont nog "Gepland" maar het plan-bestand zegt âœ… Compleet â†’ deze md is mogelijk achterhaald)
...
| B7 | MateriÃ«le tijdreizen (queryparameter geldig_op) | materiele_tijd.md |
| B8 | Aanvang/einde voor GE's en relaties (handler+struct+UI) | materiele_tijd.md |
| B9 | MateriÃ«le validatie (einde >= aanvang, geen overlap) | materiele_tijd.md |
...
| B12 | Nieuwe registratie-aanpak (Ã©Ã©n endpoint) | registration_handlers.go |
...
| B20 | âœ… Domein-tracking in registraties: `domeinen TEXT[]` kolom, afgeleide domein-set per wijziging, GIN index | nieuw |
| B21 | âœ… Backfill script bestaande registraties: `cmd/backfill_registratie_domeinen/` | nieuw |
| B22 | âœ… API domein-filter: `?domein=` queryparameter op `GET /full/registraties` met `@>` array containment | nieuw |
| B23 | âœ… GraphQL `domeinen` veld op RegistratieType | nieuw |
...
| B26 | âœ… **Sterkere typering**: uitbreiding aantal first-class datatypes (Telefoon, IBAN, GeoPunt, GeoVlak, Kleur, Bestand-FK, Bedrag, Percentage, â€¦); koppelt direct aan widget-laag (D.2) en validatie (B27). *Eerste iteratie 2026-05-13: Kleur, Duur, UrlHttps, GeoPunt toegevoegd. Tweede iteratie 2026-05-14: ISBN10, ISBN13, LEI toegevoegd. Derde iteratie 2026-05-15: DatumIncompleet, RSIN, Vestigingsnummer, Bestand, GeoLijn, GeoVlak, BAGLigplaatsID, BAGStandplaatsID toegevoegd in `model/datatype_aliases_extra.go` + `model/gegevenstypen_datatype_registry.go`. Validatiefuncties `geolijn_geojson` en `geovlak_geojson` in `model/regels_eval.go`. Unit tests in `model/validation_test.go`. Alle types gedocumenteerd in `docs/validatie.md` (40 types, alles âœ…).* | âœ… 2026-05-15 |
| B27 | ðŸŸ¡ **Validatieregels op alle typen**: per-datatype regels (regex, range, checksum, lengte) + optionele veld- en cross-veld-CEL-regels op MetaRegistry; *opt-in* uitvoering in registratie-API met response-melding (warning vs hard fail). *Fundament geleverd 2026-05-13: `model/validation.go` + walker + integratie in `RegistreerCore` met query-flag `?validatiestrengheid=strict\|lenient\|warnings-only`. Builtin: BSN-11-proef, IBAN mod-97, NLPostcode, Email, GeoPunt range, Kleur. Open: per-veld `Validatieregels`, cross-veld CEL (afh. B4), client-side preview (B6).* | brainstorm 2026-05-13 |
| B28 | **Modelvarianten verder doortesten**: combinaties materieel Ã— overerving Ã— Hub+_Data Ã— meervoudig Ã— cross-domein-relatie systematisch dekken met test-fixtures | brainstorm 2026-05-13 |
...
| B31 | **Materieel tijdreizen** (was B7) â€” eerst formele semantiek goed, daarna materieel `?geldig_op=` over de hele API + GraphQL | brainstorm 2026-05-13 |
...
| # | Item | Bron |
|---|------|------|
| D1 | createmodeltables.go â†’ meer generieke aanpak | dbsetup/createmodeltables.go |
| D2 | Generator: optie om project leeg te halen / drop tables / migreer | README.md |
| ~~D3~~ | ~~Delta-analyse CLI tool (cmd/schemadiff/)~~ **âœ… DONE** â€” zie docs/schemadiff.md | model/ontwerpkeuzen.md |
| ~~D4~~ | ~~DDL-migratiescripts genereren (ALTER TABLE ADD COLUMN)~~ **âœ… DONE** â€” schemadiff/migration.go | model/ontwerpkeuzen.md |
...
| # | Item | Bron |
|---|------|------|
| O1 | âœ… TypeMeta uitbreiden met `IsAbstract` en `ParentTypenaam` | overerving-analyse.md |
| O2 | âœ… Database + Codegen voor PFK-structuur: codegen genereert PFK-veld + belongs-to relatie op subtypes, `entiteitRelatieFieldPK` voor has-many joins met juiste PK-kolom, `createmodeltables` met topologische sort (parent-before-child) + `ensureSubtypeFK` voor FK-constraint | overerving-analyse.md |
| O3 | âœ… Generieke handlers voor supertype-join: `addOnderliggendeRelations` laadt parent via `Relation("Parent{Type}")`, `laadHubKinderenNaQuery` recursief voor parent hub-children, `ensureParentRecordBijOpvoer` maakt transparant parent-record aan in registratie-handler (TPT) | overerving-analyse.md |
| O4 | âœ… Schema-API met overervingsvelden: `IsAbstract`, `ParentTypenaam`, `GeerfdeVelden` (recursief) in DTO + builder | overerving-analyse.md |
| O5 | âœ… Editor: generalisatie-edge (driehoek-pijl) â€” rendering âœ…, sidebar-dropdown âœ…, toolbar edge-mode âœ… | overerving-analyse.md |
| O6 | âœ… Frontend: geÃ«rfde velden in formulieren. Codegen: parent belongs-to JSON tag fix (`json:"-"` â†’ `json:"parent_{lower},omitempty"`). RepresentatieFormulier: geÃ«rfde velden boven eigen velden, bewerkbaar, twee-wijziging patroon (parent vÃ³Ã³r child). EntiteitFormulier: cross-GE save met parent velden (isParentVeld marker, shared PK), standaard-weergave met aparte parent sectie. | overerving-analyse.md |
| O7 | âœ… V3 JSON roundtrip: `isAbstract` + `erft` velden op V3Entiteit, export/import generalisatie-edges | â€” |
| O8 | âœ… Codegen: schrijft `IsAbstract` + `ParentTypenaam` naar gegenereerde MetaRegistry | â€” |
| O9 | âœ… Exporters: XMI (dynamisch isAbstract + UML:Generalization), Mermaid (--|>), PlantUML (<|--) | â€” |
| O10 | âœ… Bugfix: afgeleide velden in subklassen tonen nu correct (shallow equality fix in useOvergeerfdeVelden) | â€” |
| O11 | âœ… Schemadiff: `isAbstract`-wijziging (â†’ modificatie), `erft`-wijziging (toevoegen â†’ modificatie, verwijderen/wijzigen â†’ destructief), DDL-migratie voor PFK-constraints. 7 tests. | â€” |
...
| E3 | âœ… Overerving zelf tekenen | README.md |
...
| E7 | Validatie: naamconventies, verplichte velden, referentiÃ«le integriteit | uml-editor/README.md |
| E8 | Export naar MIM linked data JSON | README.md |
| E9 | Kleur uit EA importeren | README.md |
| E10| (ook IDE) Extra REP veld "Alias" | nieuw |
| E11| âœ… Node resize: gebruiker kan nodes groter/kleiner maken (React Flow `<NodeResizer>`) + CSS max-width verwijderd | nieuw |
| E12| âœ… ENT-node dikkere rand als standaardstijl (border-width 3px vast) | nieuw |
| E13| âœ… ENTâ†’ENT edge trekken = nieuwe REL aanmaken (collapsed/ASOC-small, geen velden) + genormaliseerde handles | nieuw |
| E14| âœ… Alt-drag vanuit ENT source-handle naar canvas = nieuwe GE aanmaken, genormaliseerde edge (was Ctrl-drag, gewijzigd wegens conflict met multiSelectionKeyCode) | nieuw |
| E15| âœ… Edge-mode toolbar: Compositie (â—†) en Generalisatie (â–·) knoppen â€” selecteer mode, sleep edge, auto-reset | nieuw |
| E16| âœ… Edge-mode indicator: visuele banner + crosshair cursor bij actieve mode, Escape om te annuleren | nieuw |
| E17| âœ… rechtsklik domein wijzigen voor selectie | nieuw |
| E18| âœ… importeren uit mermaid neemt ook overerving mee | nieuw |
| E19| âœ… Auto-layout: hiÃ«rarchische ordening van alle (zichtbare) REPs op het canvas â€” ENT centraal/bovenaan, GE's eronder (rij of grid; ring bij â‰¥ 8 GE's), enums/datatypes/reflijsten als kleine boxen onder de bijbehorende GE, RELs op halverwege tussen ENT-uiteinden, ankers tussen REL en ENT, ENT-clusters per **domein** gegroepeerd in eigen blokken. Inclusief toolbar-knop `ðŸŽ¯ Auto-layout` en context-menu item `ðŸŽ¯ Auto-layout (alles)` (canvas-rechtsklik werkt nu ook zonder selectie). Implementatie in [`metamodel/autoLayout.js`](../web/vite/src/umleditor/metamodel/autoLayout.js) | nieuw |
| E20| âœ… Auto-layout: directionele ordening â€” binnen een domein worden ENT's gesorteerd op netto-flow (uitgaand vs. inkomend op REL-niveau) en met een swap-pass uitgelijnd, zodat REL-pijlen zoveel mogelijk van bronâ†’doel (linksâ†’rechts) lopen. Inclusief compactere secundair-grid (â‰ˆâˆš(N) kolommen i.p.v. lange verticale stack) bij veel enum/datatype/reflijst-instanties per ENT. | nieuw |
| E21| âœ… Auto-layout (selectie): rechtsklik bij 2+ geselecteerde nodes â†’ `ðŸŽ¯ Auto-layout (selectie)`. Reorganiseert alleen de selectie-subgraaf, gecentreerd in de bounding-box van de oorspronkelijke selectie zodat de rest van het diagram visueel op zijn plek blijft. Edges met een eindpunt in de selectie worden mee-genormaliseerd. Optie `selectie` in [`pasAutoLayoutToe`](../web/vite/src/umleditor/metamodel/autoLayout.js). | nieuw |
| E22| âœ… Sticky / locked nodes voor auto-layout: nodes met `data.layoutLocked === true` blijven door auto-layout (alles Ã©n selectie) op hun plek staan; andere nodes plaatsen zich daar omheen. Optie `respecteerLocked` (default `true`). UI-toggle (rechtsklik â†’ "ðŸ”’ Vergrendel positie") staat als follow-up open. | nieuw |
| E23| âœ… Notities & constraints landen bij hun onderwerp: auto-layout plaatst float-nodes (notitie/constraint) met dependency-edge(s) rechts naast het zwaartepunt van hun buren, met conflict-shift; notities zonder edges vallen terug op de oude rechts-stapel. | nieuw |
| E24| âœ… Auto-layout ook in de IDE: dezelfde `pasAutoLayoutToe`-implementatie wordt gebruikt door [`ide/DiagramCanvas.jsx`](../web/vite/src/ide/DiagramCanvas.jsx). Toolbar krijgt twee knoppen (`ðŸŽ¯` voor alles, `ðŸŽ¯Ë¢` voor selectie â‰¥ 2) en het rechter-muisknop-menu krijgt dezelfde items. Posities worden direct in de diagram-store opgeslagen via `updateDiagramNodes`. | nieuw |
| E25| âœ… Auto-layout â€” follow-ups doorgevoerd: (a) richting-keuze `TB`/`BT`/`LR`/`RL`/`radial` via nieuwe `richting`-optie + toolbar-dropdown in editor Ã©n IDE (gepersisteerd in `localStorage`); (b) lichte force-directed nabewerking (springs + Coulomb-repulsie, ~80 iteraties, max 25 px/stap) als laatste pass van `pasAutoLayoutToe` â€” respecteert locked nodes en (bij selectie-mode) de niet-geselecteerde nodes als pins; (c) hub-ringmodus voor ENT's met â‰¥ `hubDrempel` (=6) verbindingen waarvan de tegenpartijen in â‰¥ 2 domeinen leven: eigen RELs op een cirkel rond de hub-ENT; (d) `data.layoutLocked` toggelbaar via rechtsklik â†’ `ðŸ”’ Vergrendel positie` / `ðŸ”“ Ontgrendel positie` in editor (selectie-context-menu) en IDE (node-context-menu, valt terug op selectie). Aanvullend: scale-to-fit (`vulSelectie`) bij `Auto-layout (selectie)` zodat de subgraaf de oorspronkelijke selectie-bounding-box maximaal vult (factor â‰¤ 2.5Ã—) â€” lost het "opgepropt"-effect op. | nieuw |
| E26| âœ… Lock-status persistent gemaakt over de **volledige roundtrip** (editor â†” V3 â†” codegen â†” MetaRegistry â†” V3-export â†” editor). In de **IDE** wordt `layoutLocked` per diagram opgeslagen op `DiagramNode` (`ref.layoutLocked`), zodat een ðŸ”’ een tab-wissel of pagina-reload overleeft Ã©n per diagram verschillend kan zijn. In de **UML editor** wordt `layoutLocked` opgenomen in V3 JSON op alle node-typen (entiteit, GE, relatie, anker via `ankerLayoutLocked`, enum, gegevenstype, referentielijst-instantie, notitie, constraint). In de **backend** is `EditorLayout` uitgebreid met `LayoutLocked` + `AnkerLayoutLocked`; codegen (`gen_registry.go`, `gen_enum_registry.go`, `gen_datatypes.go`) emit deze velden in de gegenereerde `_metaregistry.go`/`_enum_registry.go`/`_datatype_registry.go` bestanden, en `v3_exporter.go` schrijft ze terug naar V3 zodat de lock-status het rebuild-traject overleeft. Notities en constraints lopen via de opgeslagen V3 JSON (geen MetaRegistry-tussenstap). Auto-layout respecteerde `data.layoutLocked` al. | nieuw |
| E27| âœ… Secundairen (enum / datatype / referentielijst) worden bij auto-layout direct **onder hun consumer-GE in dezelfde kolom** geplaatst, in plaats van in Ã©Ã©n globaal blok onderaan het ENT-cluster. RELs die zelf een secundair consumeren (bv. `Bereikbaarheid.soort â†’ Bereikbaarheidssoort`) krijgen hun secundair direct Ã³nder de REL. Dit voorkomt de lange kruisende dependency-lijnen die ontstonden wanneer BSN/Naamgebruiksoort/Bereikbaarheidssoort ver van hun owner werden gestapeld. Topologie uitgebreid met `relNaarSecundair`. | nieuw |
...
| I2 | Node toevoegen â‰  element aanmaken | docs/IDE.md |
| I3 | Node verwijderen â‰  element verwijderen | docs/IDE.md |
...
| I11 | âœ…Layout bar verplaatsbaar | README.md |
| I12 | Any bar: custom bar met functies? | README.md |
| I13 | âœ… Edge types: compositie, overerving, associatieklasse â€” rendering âœ…, toolbar edge-mode (comp+gen) âœ… | README.md |
...
| I16 | âœ…IDE toolbar: knoppen voor aanmaken nieuwe REPs (ENT, GE, REL, reflijst, type, enum) | nieuw |
| I17 | âœ…IDE toolbar + rechtsklik: normaliseer en snap-to-grid knoppen toevoegen | nieuw |
| I18 | âœ… Verplaatsbare toolbars: drag naar gewenste positie, snap verticaal bij zijranden / horizontaal bij boven-/onderrand | nieuw |
| I19 | PB rechtsklik: "Nieuw element" per type (rechtsklik op ENT â†’ nieuw GE, etc.) | nieuw |
| I20 | âœ… PB rechtsklik: "Verwijder uit model" element verwijderen inclusief alle diagrammen | nieuw |
| I21 | âœ… Domein auto-toevoegen: updateElement voegt nieuw domein automatisch toe aan domeinlijst | nieuw |
| I22 | âœ… AlignToolbar verticale layout: toolbar wisselt correct naar kolom-layout bij verticale snap | nieuw |
| I23 | âœ… Lege veldencompartimenten verbergen: ENT toont geen velden-compartiment, GE/REL tonen leeg vak i.p.v. "â€” geen velden â€”" | nieuw |
| I24 | âœ… NodeEditPanel: velden-sectie verborgen voor entiteiten (alleen afgeleide velden beschikbaar) | nieuw |
| I25 | âœ… PB diagrammen onder domeinen: domein-specifieke diagrammen verschijnen als ðŸ“ Diagrammen subfolder per domein | nieuw |
| I26 | âœ… Import-dialoog: bestand/API bron, domeinfilter, merge/vervang, auto-diagram aanmaken bij import | nieuw |
| I27 | âœ… PB rechtsklik import/export: importeer/exporteer domein met pre-ingevuld domeinfilter | nieuw |
...
| I36 | Refactor in model â†’ refactor in db? (Ook data, nu alleen structuur SQL in Delta) | nieuw |
...
| I50 | âœ… PB nesting: entâ†’ent compositie-edges produceerden dubbele entries (Plan onder OverkoepelendPlan **Ã©n** als losse entiteit) â†’ react-arborist key-collision â†’ visuele overlap bij domein-toggle. Fix: entiteit-typed children worden niet meer als sub-knoop getoond, en `idAccessor` gebruikt een composite `treeKey` voor child-rows. (2026-04-28) | nieuw |
| I51 | âœ… B5/B6/B7 (cast/splits/relatie) faalden op raw-imported elementen met "Element X is geen entiteit" omdat `rawNodeNaarElement` geen top-level `metatype` zette. Fix: metatype op top-level Ã©n in `data`; transformations.js `isEntiteit/isGE/isRelatie` accepteren nu ook `el.type` en `el.data.metatype` als fallback. Regression-test toegevoegd. (2026-04-28) | nieuw |
...
| I54 | âœ… Verplaatsen elementen in PB â†’ domein wijzigen. Rechtsklik > "â†ªï¸ Verplaats naar domeinâ€¦" Ã©n drag & drop naar domein-map. Multi-select (Ctrl+klik) wordt meegenomen. Bevestigingsdialoog; updates via `updateElement(id, { domein })`. Bugfixes: `setTimeout(0)` in onDrop (confirm-suppression in drag), `treeData` uit effect-deps (scroll-back fix), `effectAllowed="copyMove"` (drop-handler mismatch). (2026-05-02) | âœ… |
| I |  | nieuw |
| I |  | nieuw |
| I |  | nieuw |


### Frontend â€” Content Editor (Inhoud-editor)

| # | Item | Bron |
|---|------|------|
| F1 | âœ… Iteratie 2: custom formulierdefinities in JSON â€” FormulierDefinitie als bitemporale entiteit (configuratie-domein) + CustomFormulierRenderer + useFormulierDefinitie hook + integratie in EntiteitFormulier | Forms plan 02, F1-Q1Q2Q3 plan |
| F2 | âœ… Conditionele zichtbaarheid â€” `evalueerConditie()` in CustomFormulierRenderer (==, !=, truthy, falsy) | Forms plan 02, F1-Q1Q2Q3 plan |
...
| F22 | âœ… Custom tabelweergaven (WeergaveDefinitie ENT + PublicatieTabel + TabelConfig kolom-selectie/sortering) | F1-Q1Q2Q3 Fase Q2 |
| F23 | âœ… Server-side zoek/filter endpoint: `?filter.*`, `?sort=`, `?order=`, `total_count` in response | F1-Q1Q2Q3 Fase Q2 |
| F24 | âœ… Detail-pagina template renderer (PublicatieDetail met `{{veldpad}}` inserts via CEL-paden) | F1-Q1Q2Q3 Fase Q2 |
| F31 | âœ… WeergaveDefinitie bitemporale entiteit (codegen configuratie-domein: ENT + Meta/TabelConfig/DetailTemplate GE's) | F1-Q1Q2Q3 Fase Q2 |
| F32 | âœ… useWeergaveDefinitie hook + publicatie.html apart entrypoint (HashRouter, server-side paginering) | F1-Q1Q2Q3 Fase Q2 |
| F33 | âœ… Replay file: standaard WeergaveDefinities voor NatuurlijkPersoon, Initiatief, A en Land (v0.1, hub-veldnamen, definitie_versie) | F1-Q1Q2Q3 Fase Q2 |
| F25 | âœ… FormulierDefinitie bitemporale entiteit (codegen configuratie-domein: ENT + Meta GE + Layout GE) | F1-Q1Q2Q3 Fase B |
| F26 | âœ… CustomFormulierRenderer.jsx (layout JSON â†’ formulier met groep/rij/veld/conditioneel) | F1-Q1Q2Q3 Fase B |
| F27 | âœ… useFormulierDefinitie hook (fetch actieve FormulierDefinitie voor een doeltype) | F1-Q1Q2Q3 Fase B |
| F28 | âœ… EntiteitFormulier integratie: toggle custom/standaard weergave bij actieve FormulierDefinitie | F1-Q1Q2Q3 Fase B |
| F29 | âœ… Custom formulier: editable modus met cross-GE save (Ã©Ã©n registratie, meerdere GE-wijzigingen) | F1-Q1Q2Q3 Fase B vervolg |
| F30 | Visuele FormulierDefinitie layout-editor (drag-and-drop veldindeling) | F1-Q1Q2Q3 Fase B vervolg |
| F34 | âœ… JSON- en Markdown-widget: side-by-side editor + live preview, full-width grid spanning, `widget: "json"` en `widget: "markdown"` in layout | inhoud-editor-technisch.md |
| F41 | âœ… CEL evaluator uitbreiding met lijstoperaties: `celEvaluator.js` herschreven met tokenizer-ondersteuning voor `>=`, `<=`, `>`, `<`, `[`, `]`, `,`; parser met `parsePostfix` voor `.field`, `.method(args)`, `[key]`; lambda-methoden `filter(x, pred)`, `map`, `exists`, `all`; `size()`; `evaluate()` voor `index`, `methodcall`, `gt/gte/lt/lte`. `bouwCelContext` exposeert nu full actieve lijst onder `group.rolnaam` (naast enkelvoudig via `klassenaam`) zodat expressies als `Lijst.filter(x, x.veld == "v")[0].veld` werken. `childWeergave()` in `EntiteitFormulier` omgezet van minimale `ctx[klassenaam]` naar `bouwCelContext` voor uniforme lijstondersteuning. (2026-05-12) | CEL-evaluatie-js.md |
| F42 | âœ… Afgeleide velden kennis2-domein: `AfgeleideVelden` in `kennis2_metaregistry.go` gevuld voor `Kennisartikel` (`nl-titel` via `KennisartikelTaalvarianten.filter(...)`), `KennisartikelTaalvariant` (`taal` en `titel` via enkelvoudige GE-klassenaam), `Trefwoord` (`nl-trefwoord` via `Trefwoordtaalvarianten.filter(t, t.taal == "nl")[0].woord`). Alle met `IsWeergaveVeld: true`. (2026-05-12) | afgeleide-velden.md |
| F35 | GeÃ¯ntegreerde code-editor (Ã©Ã©n paneel, type "in" de gekleurde code). Opties: **react-simple-code-editor** (~3 KB, licht), **CodeMirror 6** (~150 KB, volledig), **Monaco** (~2 MB, overkill). Zie Â§12.8 in inhoud-editor-technisch.md | inhoud-editor-technisch.md |
...
### Frontend â€” Bestaande pagina's (Index/Tijdlijn/Registraties)
...
| V6 | âœ… Domein-badges en domeinfilter op RegistratieReplayPage: gekleurde chips per registratie, dropdown-filter, klikbare badges | nieuw |
...
| DM3 | âœ… Validatie vÃ³Ã³r publish: scope beperkt tot geselecteerd domein + leesbare foutmeldingen (typenamen i.p.v. indices) â€” `validateV3Model(v3, domeinFilter)` + `handleDialogChange` hervalidatie (2026-04-29) | ontwerpgedachten/domeinen |
| DM4 | âœ… Rebuild alleen voor geselecteerd domein â€” dialoog filtert validatie op gekozen domeinen (2026-04-29) | ontwerpgedachten/domeinen |
| DM5 | Domein-boundary visualisatie | ontwerpgedachten/domeinen |
| DM6 | Cross-model referentielijsten | Referentielijsten.md |
| DM7 | Domein verwijderen: flow in frontend + opschoning codegen-bestanden, `datatype_aliases.go` en `metaregistry_plumbing.go` init-calls. Basisdomein `register` kan nooit verwijderd. | DEVLOOP.md Â§3 rebuild-scenario's |
| DM8 | **Meertaligheid als domein-instelling** (uitwerking van punt 10 in 0.8): per domein aan/uit. Indien aan, krijgen *talige* content-velden op _Data een **taal-as** naast de formele tijds-as: extra kolommen `taal` (BCP 47) + `taaltype` (eigen / automatisch / juridisch / variant) op het _Data-record (geen extra tabellen). Multipliciteit-constraint: enkelvoudig in tijd Ã©n per taal â€” er kan dus per moment per taal maar Ã©Ã©n geldige waarde zijn; meervoudige GE's hebben die constraint niet. | brainstorm 2026-05-13 |
...
| R10 | **Materiele tijd in ref-lijsten**: typisch oneindig aan beide kanten, maar soms niet â€” visueel duidelijk maken (icoon / tooltip / kleur) wanneer aanvang/einde bewust beperkt zijn | brainstorm 2026-05-13 |

### Publicatie site

> Aparte Vite-entry (`publicatie.html`), eindgebruikersgerichte weergave van registerdata via WeergaveDefinities. Fundament (F22â€“F24, F31â€“F33) is âœ… gebouwd.

| # | Item | Bron |
|---|------|------|
| P1 | Meervoudige items tonen in tabelweergave (GE's met momentvoorkomen meervoudig) | nog doortesten |
| P2 | âœ… Doorzoekbare / filterbare tabelweergave voor eindgebruikers | nieuw |
| P3 | âœ… Navigatie vanuit detailpagina naar gerelateerde entiteiten (doorklikken) | nieuw |
...
| P9 | âœ… Zoeken over alle rijen: preload alle rijen | Nieuw |
...
| AUTH5 | **AuthZEN tokens experimenteren**: PEPâ†’PDP via OpenID AuthZEN evaluatie-API; vergelijken met directe Go-PDP | brainstorm 2026-05-13 |
...
| TD4 | Data-scenario's (DOC2) als bron voor TD1: scenario â†’ fixture â†’ replay-file â†’ DB | brainstorm 2026-05-13 |

### Backlog-hygiÃ«ne

| # | Item | Bron |
|---|------|------|
| BH1 | **Backlog opschonen**: meerdere items zijn al gebouwd maar niet afgevinkt. Aanpak: per increment-afsluiting backlog-pass; âœ…-marker + datum + verwijzing naar implementatie-bestand. Optioneel: `cmd/backlog-check/` script dat per item zoekt naar "BACKLOG_REF: B27" o.i.d. in code-comments en automatisch open/dicht-status bijhoudt | brainstorm 2026-05-13 |

---

## Visie & Plan â€” Increment 2
...
### Increment 2A â€” IDE verdiepen (fundamenten)

Focus: de IDE robuust en productief maken voor dagelijks modelwerk.

| Prio | Items | Reden |
|------|-------|-------|
| 1 | **I1â€“I4** Multi-diagram (tabs, node â‰  element, viewport) | Ontgrendelt werken met grotere modellen |
| 2 | **I10** Kardinaliteit [0..*] op velden | Essentieel voor correcte modellering |
| 3 | ~~**O5** Generalisatie-edge in IDE~~ **âœ… DONE** | Rendering + toolbar edge-mode knoppen |
...
| 6 | **DM2â€“DM5** Domein als actief domein, validatie, boundary | Domeinscheiding zichtbaar en afdwingbaar |
| 7 | **E11** Node resize | Gebruiker kan nodes groter/kleiner maken |
| 8 | **E13** ENTâ†’ENT edge = maak REL | Snelle relatie-creatie op canvas |
| 9 | ~~**E14** Alt-drag vanuit ENT â†’ maak GE~~ **âœ… DONE** | Snelle GE-creatie op canvas |
| 10 | ~~**I16â€“I17** IDE toolbar: create-knoppen + normaliseer/snap~~ **âœ… DONE** | Volledige IDE-werkbalk |
| 11 | ~~**E15â€“E16** Edge-mode toolbar (comp+gen)~~ **âœ… DONE** | Associatietype kiezen en tekenen |
| 11 | **I18** Verplaatsbare toolbars | Professionele IDE-layout |
| 12 | **I19** PB rechtsklik: nieuw element per type | CreÃ«ren vanuit projectbrowser |

### Increment 2B â€” Codegenerator betrouwbaar
...
| ~~3~~ | ~~**O1** âœ…, **O7â€“O10** âœ…, **O2â€“O4** âœ… Overerving in DB, handlers, schema-API~~ **âœ… DONE** | Foundations voor generalisatie |
| ~~4~~ | ~~**D3â€“D4** Delta-analyse CLI + DDL-migratie~~ **âœ… DONE** | Veilig upgraden van modellen |
| 5 | **C4** Codegen voor referentielijsten | Referentielijsten mee laten genereren |

### Increment 2C â€” Backend verrijking

Focus: de API sterker en completer maken.

| Prio | Items | Reden |
|------|-------|-------|
| 1 | **B7â€“B9** MateriÃ«le tijdreizen + validatie | Kernvaardigheid bitemporeel register |
| 2 | **B4â€“B6** Afgeleide velden (CEL in Go, validatie, API) | Modelgedreven berekende waarden |
| 3 | **B11â€“B12** Nieuwe registratie-aanpak + ongedaanmakingÂ² | Registratie-flow compleet |
...
### Increment 2D â€” Frontend: content editor iteratie 2
...
| 5 | **F10â€“F11** Records verwijderen + ongedaan maken | Basis CRUD afronden |
| 6 | **V1â€“V5** Bestaande pagina's verbeteren | Views, doorklikken, corrigeren |

### Increment 2E â€” Publicatie site
...
| **G7â€“G8** Typed mutations, subscriptions | GraphQL eerst basaal werkend |
| **CEL3** Afleiding backend-first | Eerst CEL in Go en frontend stabiel |
| **DU2â€“DU5** Cross-entity zoeken, tijdreis register-breed, export, graaf | Pas waardevol bij voldoende data en stabiele registers |
