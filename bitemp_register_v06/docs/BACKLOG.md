...
- **Menubalk per activiteit uitgebreid** (2026-06-17): submenu-ondersteuning (flyout) in `MenuBar.jsx`; verankerde volgorde in `buildMenus.js` (Bestand · [activiteit] · Beeld · Ga naar · Help). UML-IDE heeft nu **Bestand** (importeer/exporteer/upload), **Bewerken** (undo/redo + **Maak ▸** alle REP-typen + notitie/constraint), **Publiceer** (publiceer/delta/rebuild/publiceer+rebuild) en **Beeld** (nieuw diagram, herlaad, bestanden-paneel, auto-layout, **Uitlijnen ▸**). `IdePage` luistert op `uml:*`-events; `DiagramCanvas` op `uml:layout` (alleen actief diagram); `voegNieuwRepToe`/undo-redo direct op de store. DMN/BPMN/Bericht kregen eigen menu's met export.
...
- 🟢 **Fase 3B-full (typed `<Typenaam>PatchInput`, 2026-05-xx).** Het `JSON!` scalar in `wijzig<X>`/`corrigeer<X>` is vervangen door dynamisch gebouwde `*graphql.InputObject` types. Nieuw bestand [dynql/input_type_builder.go](dynql/input_type_builder.go): `BuildPatchInputTypes()` bouwt bij startup `<GETypenaam>Input` (via reflectie op `_Data`-structs) en `<Typenaam>PatchInput` (via `OnderliggendeGegevenselementen`). Materieel: `aanvang`/`einde` sub-inputs (`PlumbingDatumInput`). Relaties: secundaire FK opgenomen. Tests in [dynql/input_type_builder_test.go](dynql/input_type_builder_test.go) (8 cases). Resolvers onveranderd — graphql-go deserialiseert InputObject naar `map[string]interface{}`.
...
| B33 | **Enkelvoudig op een materieel GE = één materieel geldig record tegelijk**, niet één formeel actieve hub. De exclusieconstraint `ux_<tabel>_enkelvoudig_actief` (dbsetup) en `sluitActieveEnkelvoudigeVoorgangersAf` dwingen nu "één formeel actieve hub" af, waardoor stagen (nieuwe status met aanvang 1 oktober naast de huidige) onmogelijk is en de geschiedenis niet formeel actueel kan blijven (vgl. woongeschiedenis: actueel, maar één adres tegelijk). Gewenst: constraint op niet-overlappende materiële periodes; de opvolger-afsluiting zet dan het materiële einde i.p.v. formele afvoer. De uitvoerder van opgeslagen documenten kiest al per GE het materieel geldige record (`dynql.kiesGeldigeHub`) en werkt in beide werelden. Zie plan 2026-09-22 §7.4.2 | Mark/Claude 24-09-2026 |
| B34 | **Formulier-niveau validatie voor lijsten (min-rijen, verplichte vaste rij)**. In nieuw-modus wordt een lijstrij waarin buiten de vaste waarden niets is ingevuld overgeslagen; `min` op een lijst stuurt alleen de weergave (virtuele rijen), niet de verplichting. Het aanmeldformulier vraagt een contactorganisatie (*) die nu leeg mag blijven. Gewenst: `min` ook als validatie in `bouwNieuwWijzigingen` (→ `ontbrekend`), en de verplichte-veld-meldingen bij het veld tonen i.p.v. alleen onder de knop. Zie plan 2026-09-22 §9 | Claude 24-09-2026 |
| B35 | **`Initiatiefinfo.PbiID` verplicht blokkeert vraag 30 van het aanmeldformulier**. `informatie` en `PbiID` zijn beide verplicht (niet-pointer in het CG-model); een vrije opmerking bij de aanmelding kan daardoor niet in `Initiatiefinfo` zonder een PBI-nummer. Modelkeuze: `PbiID` optioneel maken, of de opmerking in het aanmeld-logboek (plan §6.4) opnemen | Claude 24-09-2026 |
| B36 | **`EntiteitCombobox` laadt de hele `/full`-lijst van de doelentiteit** (gepagineerd, max 2000) om labels via het weergaveveld te berekenen; voor Organisatie (~130) prima, voor grote ENT's niet. Gewenst: een server-side zoekendpoint voor gewone entiteiten zoals `/api/viz/reflijst/:type/opties` (met `q=`), of `secondaire-ids` uitbreiden met weergavenamen | Claude 24-09-2026 |
...
| E24| ✅ Auto-layout ook in de IDE: dezelfde `pasAutoLayoutToe`-implementatie wordt gebruikt door [`ide/DiagramCanvas.jsx`](../web/vite/src/ide/DiagramCanvas.jsx). Toolbar krijgt twee knoppen (`🎯` voor alles, `🎯ˢ` voor selectie ≥ 2) en het rechter-muisknop-menu krijgt dezelfde items. Posities worden direct in de diagram-store opgeslagen via `updateDiagramNodes`. | nieuw |
...
| PE4 | check-locatie topic — GET /full/locaties/{id} met bestaat/actueel variabelen | ✅ |
...
| PE8 | bron/bron_kenmerk Operaton-provenance in elke registratie (zie B32) | ✅ |
