...
- 🟡 **Fase 3B-full (typed `<Type>OpvoerInput`, openstaand).** Recursieve graphql-input-types per representatie (gegenereerd uit MetaRegistry). Typed mutations voor GE/REL met `(entId, relId)`-args. Bestaande JSONScalar-mutations blijven voor back-compat.
