...
func TestEqualStructComparablePointers(t *testing.T) {
	...
	a1 := A_U{Rel_ID: 1, Opvoer: &now, Afvoer: nil}
	a2 := A_U{Rel_ID: 1, Opvoer: &now, Afvoer: nil}
	// same pointer address -> Equal should be true
	if !Equal(a1, a2) {
		t.Fatalf("expected a1 == a2 (same pointer): %#v %#v", a1, a2)
	}

	a3 := A_U{Rel_ID: 1, Opvoer: &copyNow, Afvoer: nil}
...
func TestEqualForB(t *testing.T) {
	now := time.Now()
	b1 := B_X{Rel_ID: 1, Opvoer: &now}
	b2 := B_X{Rel_ID: 1, Opvoer: &now}
	if !Equal(b1, b2) {
